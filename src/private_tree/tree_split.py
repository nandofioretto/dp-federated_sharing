...
class DP_Random_Forest:
    ...
    def __init__(self,
        ...
        ''' Some initialization '''
        self._categs = categs
        self._data  = pd.DataFrame(train)
        self._ydata = train[:,0].astype(int)
...
class Tree(DP_Random_Forest):
    ...
    def get_split(self, attr, splits, domain):
        d = self.data
        props, scores = [], []
        for k, (l,u) in domain.items():
            if k == attr: continue
            props.append((d[int(k)] >= l) & (d[int(k)] <= u))
        props.append([True] * len(props[-1]))
        props = np.asarray(props)
        l, u = domain[str(attr)]
        for split in splits:
            props[-1] = (d[attr] >= l) & (d[attr] < split)
            y1 = np.bincount(self.ydata[props.all(axis=0)], minlength=len(self.class_labels))
            props[-1] = (d[attr] >= split) & (d[attr] <= u)
            y2 = np.bincount(self.ydata[props.all(axis=0)], minlength=len(self.class_labels))
            scores.append(max(y1) + min(y2))

        return splits[np.argmax(scores)]
    ...
    def __init__(self, attribute_indexes, root_attribute, pc, seed=1234):
        ...
        self.seed = seed
        self.rand = np.random.RandomState(seed)
        self.class_labels = pc.class_labels
        self.data = pc._data
        self.ydata = pc._ydata
        self.domains = pc._attribute_domains
        ...
        if root_attribute not in self._categs:  # numerical attribute
            ###
            lb, ub = attribute_domains[str(root_attribute)]
            split_vals = self.rand.uniform(lb, ub, 10)
            split_val = self.get_split(root_attribute, split_vals, self.domains)
    ...
    def make_children(self, candidate_atts, parent_node, current_depth, splitting_value_from_parent, svfp_numer,
        ...
        if not candidate_atts or current_depth >= self._max_depth + 1:  # termination conditions. leaf nodes don't count to the depth.
            ...
            if new_splitting_attr not in self._categs:  # numerical attribute
                ####
                lb, ub = attribute_domains[str(new_splitting_attr)]
                split_vals = self.rand.uniform(lb, ub, 10)
                split_val = self.get_split(new_splitting_attr, split_vals, attribute_domains)
